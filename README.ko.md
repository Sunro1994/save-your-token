...
메시지를 전송하기 전에 토큰 낭비가 예상되는 패턴을 감지하고 더 효율적인 요청 방법을 제안합니다.
요청 자체는 차단하지 않으며 경고 메시지만 출력합니다.

감지 패턴: 전체 파일 요청, 처음부터 재작성, 전면 리팩토링, 모든 에러 수정, 범위 불명확 요청, 프로젝트 전체 범위 등
...
Claude 응답이 끝날 때 하루 1회 토큰 사용 스냅샷을 자동으로 저장합니다.
저장 위치: `~/.claude/token-diet-reports/YYYY-MM-DD.json`
`/token-diet` 실행 시 이 데이터를 읽어 7일 트렌드를 비교해 보여줍니다.
당일 파일이 이미 있으면 저장하지 않습니다.
...
- prompt-lint 훅: 매 `UserPromptSubmit` 이벤트마다 메시지를 고비용 패턴과 대조하고 리라이팅 제안을 stderr로 출력합니다. heredoc stdin 충돌 방지를 위해 Python 코드를 임시 파일에 기록한 뒤 실행합니다.
- context-watch 훅: 매 `Stop` 이벤트마다 최신 세션 JSONL을 읽어 200k 토큰 한도 대비 사용률을 계산합니다.
- report-save 훅: 매 `Stop` 이벤트마다 실행되며, 당일 리포트가 없을 때 한 번만 JSON 스냅샷을 저장합니다.
...
- 일별 리포트 저장을 활용한 7일 트렌드 비교
- ReadOnce 훅 SHA-256 해시 적용 및 stdin 파싱 버그 수정
- prompt-lint 훅 — 전송 전 토큰 낭비 패턴 감지 및 리라이팅 제안
- context-watch 훅 — 컨텍스트 임계값 자동 경고
- report-save 훅 — 일별 토큰 사용량 스냅샷 자동 저장
- 한국어 전용 인터페이스
